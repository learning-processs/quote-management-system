  ...
  return (
    <aside className={`w-52 flex-shrink-0 border-r ${theme.border} ${theme.bg} flex flex-col min-h-screen transition-colors duration-300`}>
      ...
      <div className={`px-5 py-5 border-b ${theme.border}`}>
        ...
        <span className="text-xs text-blue-500 font-medium uppercase tracking-widest mt-1 block">
          Admin Panel
        </span>
      </div>
      ...
      <div className={`px-5 py-5 border-b ${theme.border}`}>
        ...
        <div className="text-xs text-blue-500 font-medium mt-0.5">{admin?.role}</div>
      </div>
      ...
      {/* Theme toggle + Logout */}
      <div className={`px-5 py-5 border-t ${theme.border} space-y-3`}>
        <button
          onClick={toggleTheme}
          className={`flex items-center gap-3 text-sm ${theme.text2} hover:text-blue-500 transition w-full`}
        >
          <span>{dark ? '☀️' : '🌙'}</span>
